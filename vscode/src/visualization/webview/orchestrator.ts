...
let lastPillarStats = {};
    ...
    function graphToCytoscapeElementsForGeneralView(graph) {
        ...
        const nodeType = (n: { type?: string; element_type?: string }) => ((n?.type || n?.element_type || '') as string).toLowerCase();
        const isPartDef = (n: { type?: string; element_type?: string }) => n && nodeType(n).includes('part def');
        const isPartUsage = (n: { type?: string; element_type?: string }) => n && (nodeType(n) === 'part' || nodeType(n).includes('part usage'));
        const isPartOrPartDef = (n: { type?: string; element_type?: string }) => isPartDef(n) || isPartUsage(n);
        const nodeById = new Map<string, { type?: string; element_type?: string }>();
        nodes.forEach((n) => { if (n && n.id) nodeById.set(n.id, n); });
        ...
        const getEdgeType = (e: { type?: string; rel_type?: string }) => ((e.type || e.rel_type || '') as string).toLowerCase();
        const rawPartEdges = rawContainsTypingSpecializes
            ...
            .map((e) => {
                ...
                const type = t === 'contains' ? 'hierarchy' as const : t === 'typing' ? 'typing' as const : t === 'specializes' ? 'specializes' as const : 'hierarchy' as const;
        ...
        edgesToUse.forEach((e) => { edgeEndpointIds.add(e.source); edgeEndpointIds.add(e.target); });
        ...
        const specializesEdgeIds = new Set();
