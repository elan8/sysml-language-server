...
const VIEW_OPTIONS: { value: DiagramView; label: string }[] = [
...
export class VisualizationPanel {
  ...
  static createOrShow(
    ...
    log("VisualizationPanel.createOrShow", "focusPackage:", focusPackageName, "files:", workspaceFileUris?.length ?? 0);
  ...
  async refresh(): Promise<void> {
    log("VisualizationPanel.refresh", "workspaceFiles:", this.workspaceFileUris.length);
    ...
    if (!doc || (doc.languageId !== "sysml" && doc.languageId !== "kerml")) {
      log("VisualizationPanel.refresh: no active SysML/KerML doc");
    ...
    try {
      ...
      log("VisualizationPanel.refresh: sent model", elements.length, "elements,", relationships.length, "relationships");
    } catch (e) {
      logError("VisualizationPanel.refresh failed", e);
      this.panel.webview.postMessage({
        command: "setModel",
  ...
  private async refreshFromWorkspaceUris(): Promise<void> {
    const uris = this.workspaceFileUris;
    log("VisualizationPanel.refreshFromWorkspaceUris:", uris.length, "files");
    const allElements: SysMLElementDTO[] = [];
    const allRelationships: RelationshipDTO[] = [];
    const relKeys = new Set<string>();

    try {
      const results = await Promise.all(
        uris.map((uri) =>
          this.modelProvider.getModel(uri.toString(), [
            "elements",
            "relationships",
            "stats",
          ])
      ...
      log("VisualizationPanel.refreshFromWorkspaceUris: sent", merged.length, "merged elements");
    } catch (e) {
      logError("VisualizationPanel.refreshFromWorkspaceUris failed", e);
      this.panel.webview.postMessage({
        command: "setModel",
  ...
  private mergeNamespaceElements(elements: SysMLElementDTO[]): SysMLElementDTO[] {
    ...
    for (const el of elements) {
      ...
      if (namespaceTypes.has(el.type) && mergedMap.has(key)) {
        ...
        if (idx !== -1) result[idx] = mergedMap.get(key)!;
  ...
  private countElements(elements: SysMLElementDTO[]): number {
    const rec = (e: SysMLElementDTO): number =>
      1 + (e.children ?? []).reduce((s, c) => s + rec(c), 0);
  ...
  private async onMessage(msg: any): Promise<void> {
    ...
    if (msg.command === "setView" && msg.view) {
      log("VisualizationPanel: setView", msg.view);
      this.currentView = msg.view as DiagramView;
      await this.refresh();
      return;
    }
    ...
    if (msg.command === "export" && msg.format && msg.data !== undefined) {
      const format = msg.format as "png" | "svg";
      log("VisualizationPanel: export", format);
      ...
      if (uri) {
        if (format === "png") {
          const bytes = Buffer.from(msg.data as string, "base64");
          await vscode.workspace.fs.writeFile(uri, bytes);
        } else {
          const bytes = Buffer.from(msg.data as string, "utf8");
          await vscode.workspace.fs.writeFile(uri, bytes);
        }
  ...
  private renderHtml(extensionUri: vscode.Uri): string {
    ...
    const elkUri = this.panel.webview.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, "node_modules", "elkjs", "lib", "elk.bundled.js")
    );
    const cytoscapeSvgUri = this.panel.webview.asWebviewUri(
      vscode.Uri.joinPath(extensionUri, "node_modules", "cytoscape-svg", "cytoscape-svg.js")
    );
