...
class VisualizationPanel {
    ...
    async _doUpdateVisualization() {
        try {
            ...
            const results = await Promise.all(urisToQuery.map(uri => this._lspModelProvider.getModel(uri, scopes)));
     ...
     */
    convertDTOElementsToJSON(elements, parentName) {
     ...
     */
    static mergeElementDTOs(elements) {
    ...
    async jumpToElement(elementName, skipCentering = false, parentContext) {
        this._isNavigating = true; // Set navigation flag
        let element;
        const dto = await this._lspModelProvider.findElement(this._document.uri.toString(), elementName, parentContext);
        if (dto) {
    ...
    findElementRecursive(name, elements) {
     ...
     */
    findElementInParent(elementName, parentName, elements) {
    ...
    async renameElement(oldName, newName) {
        // Validate new name
        ...
        let element;
        const dto = await this._lspModelProvider.findElement(this._document.uri.toString(), oldName);
        if (dto) {
            element = {
                type: dto.type,
                name: dto.name,
        ...
        const nameRange = new vscode.Range(this._document.positionAt(nameStartOffset), this._document.positionAt(nameEndOffset));
        // Apply the edit
